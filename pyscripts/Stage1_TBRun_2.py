#########
#  loading few needed packages
import csv, pickle, operator, os, sys , pp              
import collections 

#python /media/Big/Work/WorkVLA/Projects/RD0070/pipeline/code/Woking/Stage1_TBRun_1.py /media/Big/Work/WorkVLA/Data/TB_Runs/TB_Runs_Results /media/Big/Work/WorkVLA/Projects/RD0070/pipeline/stage1/20131220_223Strains/patterns AF2122 20140530_SB4020__WGS-TB-20140521 1 8
#python /media/Big/Work/WorkVLA/Projects/RD0070/pipeline/code/Woking/Stage1_TBRun__linux_1.py /media/Big/Work/WorkVLA/Data/TB_Runs/TB_Runs_Results /media/Big/Work/WorkVLA/Projects/RD0070/pipeline/stage1/20131220_223Strains/patterns AF2122 20140108_SB4020__WGS-TB-20131218 2 8
#########
#   parsing the arguments
args=sys.argv

if len(args)<2:
    pathTBRuns="/media/javier/3TB/Work/WorkVLA/Data/TB_Results/WGS_TBResults_try"
    pathPatterns="/media/javier/3TB/Work/WorkVLA/Software/WGS_BTB/Stage1_patterns"
    refName="AF2122.fna"
    TBRun ="20180406_SB4020__sub391-G49-17"
    ncpus=1
    qth=8
    thMinGoodCov=2
    thCovProp=0.2
...
# reads the fastq format genome files (fq files)
# resturn a string with the genome in the fq file
def readfqFile(fname):
...
def findGenotypeOneSample(strainsDetailsTittle,strainDetails,pathTBRuns,patternsDetails,patternsBritishBTBDetails,patternsBTBDetails,patternsMic_PinDetails,patternsMicrotiDetails,patternsPinnipediiDetails,refName,qth,pathAux,thMinGoodCov,thCovProp,thqualsnp,thqualnonsnp):    
    prunName=strainsDetailsTittle.index('runName')
    pmeanCov=strainsDetailsTittle.index('meanCov')
    pfileName=strainsDetailsTittle.index('fileName')
    #pstrain_id=strainsDetailsTittle.index('strain_id')
    #pgenotype=strainsDetailsTittle.index('genotype')
    name=[strainDetails[pfileName]]
    meanCov=float(strainDetails[pmeanCov])
    #if strainDetails[pstrain_id]=="NA":
    #name=[strainDetails[pfileName]+"-"+strainDetails[pgenotype]]
    #else:
    #    name=[strainDetails[pstrain_id]+"-"+strainDetails[pgenotype]]
    print "Procesing "+strainDetails[pfileName]
    
    strainStatsFileName=os.path.join(pathTBRuns,strainDetails[prunName],refName,"Mapped",strainDetails[pfileName],strainDetails[pfileName]+".pileup.vcf.gz")
...
strainDetailsFile=os.path.join(pathTBRuns,TBRun,refName,"Mapped",TBRun+"_Mapped_"+refName+"_QC.csv")
pathResutls=os.path.join(pathTBRuns,TBRun,"Stage1")
...
pfileName=strainsInfo[0].index('fileName')
...
ppermap=strainsInfo[0].index('per_map')
totalReads=strainsInfo[0].index('total_reads')
strainsInfo=listT(strainsInfo)
strainsDetails=listT([strainsInfo[pfileName][1:],strainsInfo[prunName][1:],strainsInfo[pequalSites][1:],strainsInfo[pmeanCov][1:],strainsInfo[totalReads][1:],strainsInfo[ppermap][1:],strainsInfo[preadLen][1:]])

strainsDetails=[['fileName','runName','per_equalSites','meanCov','totalReads','perMapped','AverageReadLength']]+strainsDetails
...
outFileName=os.path.join(pathResutls,TBRun+"_stage1.csv")
