...
if len(args)<2:
    #pathTBfastqs=(os.path.dirname(os.getcwd()))
    pathTBRunsResutls="/media/Second3TB/Work/WorkVLA/Data/TB_Runs/TB_Results"
    pathPatterns="/home/richard/MyScripts/BovTB-nf/references/Stage2_patterns"
    TBRun ="test"
    ncpus=1
    qth=8
    stage1="false"
...
def typeOneStrain(strainsDetailsHeader,strainDetails,spoPatterns,pathTBfastqs,pathPatterns,pathAux,wl,pathCrunchs,ppredType,thHitLen,thHitSim,qth,spth,spCountTh,stage1):
    ...
    if meanCov>=qth:
        if flag not in ["BritishbTB","nonBritishbTB"] or stage1=="false":
            ...
            #strainFastqPath=os.path.join(pathTBfastqs) ***change this bit to input trim fastq
...
strainDetailsFile=os.path.join(TBRun,"Stage1",TBRun+"_stage1.csv")
if stage1=="true":
    pathResutls=os.path.join(TBRun,"Stage1_Stage2")
else:
    pathResutls=os.path.join(TBRun,"Stage2")
